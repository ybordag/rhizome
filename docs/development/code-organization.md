...
| `nodes.py` | All node implementations: `session_context_intake`, `weather_context_loader`, `triage_reasoner`, `llm_call`, `interaction_node`, `tool_node`. It also owns the system prompt template, prompt-section assembly, pinned-context prompt summaries with `[id: ...]` markers, `DESTRUCTIVE_TOOLS`, `INTERACTION_REVIEW_TOOLS`, and routing functions (`should_continue`, `should_continue_after_interaction`). |
...
| `session_context.py` | Text-first startup/session context helpers for Verdant: empty/unset response shape, inferred/user source normalization, partial PATCH application, graph-state merge, focus object label resolution, and prompt-summary formatting. Prompt summaries include focus-object ids in `[id: ...]` form so the model can make exact tool calls. |
