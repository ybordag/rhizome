...
The Rhizome API is consumed by Cambium (the Go gateway) which proxies it to Verdant (the React frontend) under `/api/v1`. All requests include a `user_id` extracted from the verified JWT by Cambium — Rhizome never handles auth directly.

This document describes the intended API surface. The backing tools exist in Rhizome; the Cambium proxy layer is in development.
