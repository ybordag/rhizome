...
class Thread(Base):
    """
    Conversation thread metadata. The id IS the LangGraph thread_id — a
    botanical three-word name generated by Cambium (e.g. 'silver-fern-cascade').
    Actual message content lives in the LangGraph PostgresSaver checkpointer.
    """
    __tablename__ = "thread"
    __table_args__ = (
        Index("ix_thread_user_id", "user_id"),
        Index("ix_thread_last_active_at", "last_active_at"),
    )
