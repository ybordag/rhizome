...
**Next in Rhizome — Thread management (narcissus branch):**

New `Thread` model in `db/models.py`:
```
id                   str PK       — IS the LangGraph thread_id (botanical name from Cambium)
user_id              int FK+index
title                str nullable  — auto-set from first message or user-defined
project_id           str FK nullable
last_message_preview str nullable  — first 100 chars of last AI response
last_active_at       datetime nullable
message_count        int default=0
created_at           datetime
```

`session_context_intake` in `agent/core/nodes.py` auto-updates `last_active_at`
and `last_message_preview` after each turn (upsert on thread_id).

New data endpoints in `agent/api/routers.py`:
```
POST   /internal/data/threads                 — register {id, user_id, title?}
GET    /internal/data/threads                 — list user's threads (last_active_at desc)
GET    /internal/data/threads/{id}            — metadata + preview
GET    /internal/data/threads/{id}/messages   — history from LangGraph checkpoint
DELETE /internal/data/threads/{id}            — delete metadata + clear checkpoint
```

Interrupted stream recovery: LangGraph checkpoint always stores the complete AI
response (saved after `llm_call` node completes, before SSE streaming to client).
On reconnect, GET .../messages returns full history — no Redis or separate cache.

Thread IDs are generated by Cambium as three-word botanical names
(e.g. `silver-fern-cascade`). Rhizome stores and uses them as opaque strings.

**Also pending:**
- Multi-tenancy: audit all tool queries for `user_id` scoping (after Cambium Phase 4)
- Pest intelligence (deferred from calendula Phase 5): iNaturalist + image-based pest ID + RAG — after visual garden understanding
